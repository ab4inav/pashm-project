...
export default function Footer() {
  return (
    <footer className="bg-deep-earth text-ivory/80 relative z-10">
      {/* Woven top border */}
      <div className="h-px bg-repeating-linear-gradient" style={{
        background: "repeating-linear-gradient(90deg, #C4722A 0px, #C4722A 4px, transparent 4px, transparent 8px)",
        opacity: 0.6,
      }} />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-16 lg:py-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
          ...
          <div className="md:col-span-1">
            ...
            <p
              className="text-ivory/50 text-sm italic mb-6"
              style={{ fontFamily: "var(--font-serif)" }}
            >
              Where the mountains meet the loom,
              <br />
            ...
            <div className="flex flex-col gap-3">
              {footerLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm text-ivory/60 hover:text-saffron-light transition-colors duration-300"
                >
                  {link.label}
                </Link>
              ))}
        ...
        <div className="mt-16 pt-8 border-t border-ivory/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs text-ivory/30">
            &copy; {new Date().getFullYear()} The Pashm Project. Handcrafted in
            Basohli.
          </p>
          <p
            className="text-xs text-ivory/20 italic"
            style={{ fontFamily: "var(--font-serif)" }}
          >
            Every thread has a name. Every shawl has a story.
          </p>
        </div>
      </div>
    </footer>
